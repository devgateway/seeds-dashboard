...
import {
    DATA, WP_CATEGORIES, WP_DOCUMENTS, DATA_CATEGORY, WP_CROPS
...
const CountryReports = (props) => {
    ...
    const view = intl.formatMessage({id: "view", defaultMessage: "View"});
    ...
    useEffect(() => {
        if (categoriesWP && !loading && !documentsAndImages) {
            onLoadDocuments({})
        }
    ...
    const generateLinks = () => {
        if (documentsAndImages && categoriesWP) {
            const docs = documentsAndImages.filter(d => d.mime_type === 'application/pdf'
                ...
                && d.slug.includes(titleKey.toLowerCase())
            );
            if (docs.length === 0) {
                return null;
            }
    ...
    const cleanupCountryName = (name) => {
        ...
        return intl.formatMessage({id: normalizeField(newName)});
    }
...
export default connect(mapStateToProps, mapActionCreators)(injectIntl(CountryReports))

...
const ListOfDocuments = ({
    ...
    if (documents && selectedCountryId && countries && category !== '0') {
        ...
        const countryCategory = categoriesWP.find(i => {
            return i.slug.toLowerCase() === (selectedCountry.country.toLowerCase().replace(/\s+/g, '-').toLowerCase() + documentSlugPostFix) && i.parent === Number(category);
        })
        if (countryCategory) {
            filtered = documents.filter(i => i.categories.find(j => j === countryCategory.id)
                && i.categories.find(j => j === categoriesWP.find(k => k.name === locale).id));
        }
    }

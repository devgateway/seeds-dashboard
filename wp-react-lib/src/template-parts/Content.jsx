...
class Content extends React.Component {
    ...
    render() {
        ...
        if (post) {
            ...
            let pReadMore = "Read More";
            if (readMore) {
                pReadMore = readMore;
            }
            if (messages && messages[locale]['read-more']) {
                pReadMore = messages[locale]['read-more'];
            }
                ...
                <Enhance className="entry-content" {...this.props}>
                    ...
                    {showLink === true && showLinksInModal !== "true" &&
                        <a href={isAddTypeToLink ? this.replaceSlugWithId(replaceLink(post.link, locale, isAddTypeToLink), post.slug, post.id) : replaceLink(post.link, locale, isAddTypeToLink)}
                           className="link">{pReadMore}</a>}
                    {showLink === true && showLinksInModal === "true" &&
                        <a className="link" style={{ cursor: 'pointer' }}
                           onClick={() => this.switchModal()}>{pReadMore}</a>}
                    {showLink === true && showLinksInModal === "true" &&

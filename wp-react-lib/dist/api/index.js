...
export const getPosts = (slug, type, taxonomy, categories, before, perPage, page, fields, locale, previewNonce, previewId, search) => {
  ...
  url += '_embed=true&lang=' + locale + (slug ? '&slug=' + slug : '');

  if (!slug) {
    url += (categories ? (taxonomy ? '&' + taxonomy : '&categories') + "=" + (categories ? categories : "") : '') + ( //ids
    before ? "&before=" + before.toISOString() : "") + (perPage ? '&per_page=' + perPage : '') + (page ? '&page=' + page : '') + (fields ? '&_fields=' + fields : '') + (search ? '&search=' + search : '');
  }
...
export const getPages = (before, perPage, page, fields, parent, slug, store, locale, previewNonce, previewId, search) => {
  ...
  url += 'lang=' + locale + (slug ? '&slug=' + slug : '');

  if (!slug) {
    url += (before ? "&before=" + before.toISOString() : "") + (perPage ? '&per_page=' + perPage : '') + (page ? '&page=' + page : '') + (fields ? '&_fields=' + fields : '') + (parent ? '&parent=' + parent : '') + (search ? '&search=' + search : '');
  }

  return get(url);
};
export const search = (context, page, perPage, search, type, subtype, locale) => {
  let url = URL_SEARCH + '?lang=' + locale + (context ? "&context=" + context : '') + (perPage ? '&per_page=' + perPage : '') + (page ? '&page=' + page : '') + (search ? '&search=' + search : '') + (type ? '&type=' + type : '') + (subtype ? '&subtype=' + subtype : '');
  return get(url);
};
